...
export default function LessonsPage() {
  ...
  // ── When course changes, filter modules ────────────────────
  const handleCourseChange = (courseId: string) => {
    setSelectedCourseId(courseId);
    setFilteredModules(modules.filter((m) => m.course_id === courseId));
    setForm((prev) => ({ ...prev, module_id: "" })); // reset module selection
  };

  // ── Open edit form ─────────────────────────────────────────
  const handleEditClick = (lesson: Lesson) => {
    // Find which course this lesson's module belongs to
      ...
      {showForm && (
        <div className="rounded-xl bg-white border border-gray-200 shadow-sm p-6 space-y-4">
          ...
          <div className="space-y-4">
            ...
            {/* Step 2: Pick Module (filtered by course) */}
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-2">
